import { type Boss, type BossConfig, calculateBossTrace } from "@/utils/genesis";
...
export const GenesisBossSelector: React.FC<Props> = ({ boss, config, onChange }) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 max-[600px]:gap-1.5 py-1.5 px-2 max-[600px]:px-1.5 rounded-md bg-slate-400/30 dark:bg-white/20 hover:bg-slate-300/70 dark:hover:bg-white/10 transition-colors text-xs">
      ...
      />
      <p className="min-w-[50px] max-[600px]:min-w-[40px] font-bold text-gray-900 dark:text-gray-100 truncate">{boss.name}</p>
      <select
