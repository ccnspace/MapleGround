...
export const ReportContainer = ({ nickname }: { nickname: string }) => {
  ...
  return (
    <>
      <div className="flex flex-col items-center gap-3 mt-8 mb-8 px-2 min-w-[1280px] max-[600px]:min-w-0 max-[600px]:px-2">
        {/* 캐릭터 카드 컨테이너 */}
        <div className="flex flex-col items-center w-full max-w-4xl">
          {/* 캐릭터 카드 2개 - 항상 가로 배치 */}
          <div className="flex flex-row items-start justify-center gap-6 w-full max-[600px]:gap-3">
            <div className="w-[360px] flex-shrink max-[600px]:flex-1 max-[600px]:w-auto max-[600px]:min-w-0">
              <h3 className="text-lg font-bold text-slate-700 dark:text-slate-200 mb-2 text-center max-[600px]:text-sm">
                첫 번째 캐릭터
              </h3>
              <CharacterCard
                type="first"
          ...
          {/* 비교 버튼 - 아래 가운데 */}
          <div className="flex justify-center mt-6 max-[600px]:mt-4">
            <button
              onClick={handleClick}
              className="w-[180px] px-8 py-2.5 rounded-3xl text-base font-semibold text-white
                ...
                max-[600px]:w-[200px] max-[600px]:text-sm max-[600px]:py-2"
            >
              <span className="flex items-center justify-center gap-2 font-bold text-xl max-[600px]:text-base">
                <span>비교하기</span>
              </span>
            </button>
          </div>
        ...
        {/* 비교 리포트 - 모바일에서 세로 배치 */}
        <div className="flex flex-row gap-6 mt-6 max-[600px]:flex-col max-[600px]:w-full max-[600px]:gap-4">
          {/* 간단 비교 리포트 */}
          {versusSimpleReport.length > 0 && (
            <ReportCard className="h-[500px] max-[600px]:h-[350px] max-[600px]:w-full overflow-y-auto">
          ...
          {versusDetailReport && (
            <>
              {/* 많이 성장시킨 아이템 */}
              <ReportCard className="h-[500px] max-[600px]:h-[350px] max-[600px]:w-full">
                ...
                {versusDetailReport.positiveScores.length === 0 && (
                  <div className="flex items-center justify-center h-[calc(100%-3.5rem)]">
                    <p className="text-slate-500 dark:text-slate-400 max-[600px]:text-sm">나를 성장시킨 아이템이 없습니다.</p>
                  </div>
                )}
              </ReportCard>
              ...
              <ReportCard className="h-[500px] max-[600px]:h-[350px] max-[600px]:w-full">
                ...
                {versusDetailReport.negativeScores.length === 0 && (
                  <div className="flex items-center justify-center h-[calc(100%-3.5rem)]">
                    <p className="text-slate-500 dark:text-slate-400 max-[600px]:text-sm">성장을 방해한 아이템이 없습니다.</p>
                  </div>
                )}
              </ReportCard>
            </>
