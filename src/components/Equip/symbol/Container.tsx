...
export const SymbolContainer = ({ type, item }: Props) => {
  ...
  const growthRatio = `${Math.round((symbol_growth_count / symbol_require_growth_count) * 100)}%`;
  const growthInfo = isMaxLevel ? "MAX" : `${symbol_growth_count} / ${symbol_require_growth_count} (${growthRatio})`;
  ...
  return (
    <>
      ...
      <div className="flex flex-col text-xs font-medium gap-0.5">
        ...
        {symbol_level && <p className="text-yellow-500">성장치: {growthInfo}</p>}
        {symbol_exp_rate !== "0%" && <p className="text-white">경험치 획득량 : +{symbol_exp_rate}</p>}
        {symbol_meso_rate !== "0%" && <p className="text-white">메소 획득량 : +{symbol_meso_rate}</p>}
        {symbol_drop_rate !== "0%" && <p className="text-white">아이템 드롭률 : +{symbol_drop_rate}</p>}
        ...
        {symbol_force !== "0" && <p className="text-white">{`${forceLabel} : +${symbol_force}`}</p>}
      </div>
      <Divider />
      <div className="flex text-xs font-light whitespace-pre-wrap text-white">{symbol_description}</div>
    </>
  );
};
